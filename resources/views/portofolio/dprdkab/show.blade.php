...
<head>
    ...
    <title>DPRD Kabupaten | Biodata Caleg</title>
...
<body>
  <nav class="navbar navbar-expand-lg bg-body-tertiary">
    ...
    <main class="container" style="margin-top: 50px;">
        <h1 style="margin-bottom: 30px">Data Caleg DPRD Kabupaten</h1>
        {{-- <a href="{{ route('edit', $form_dprdkab->id) }}" class="btn btn-primary">Edit Portofolio</a> --}}
        <div class="row">
            <div class="col-md-6">
               ...
               @if($form_dprdkab->foto)
                   <img src="{{ Storage::url($form_dprdkab->foto) }}" alt="kenapasih" class="bd-placeholder-img rounded-circle" width="270" height="270" style="margin-top: 50px; margin-left: 40px;" >
               @else
            ...
            <div class="col-md-6" style="margin-left: -130px">
              <div class="mb-3">
                ...
                <input class="form-control" type="text" value="{{ $form_dprdkab->nama }}" aria-label="Disabled input example" disabled readonly>
              </div>
              <div class="mb-3">
                <label for="formGroupExampleInput" class="form-label">Jenis Kelamin</label>
                <input class="form-control" type="text" value="{{ $form_dprdkab->jenis_kelamin }}" aria-label="Disabled input example" disabled readonly>
              </div>
              <div class="mb-3">
                <label for="formGroupExampleInput" class="form-label">Kewarganegaraan</label>
                <input class="form-control" type="text" value="{{ $form_dprdkab->kewarganegaraan }}" aria-label="Disabled input example" disabled readonly>
              </div>
              <div class="mb-3">
                <label for="formGroupExampleInput" class="form-label">Tempat Tanggal Lahir</label>
                <input class="form-control" type="text" value="{{ $form_dprdkab->tempat_tanggal_lahir }}" aria-label="Disabled input example" disabled readonly>
              </div>
              <div class="mb-3">
                <label for="formGroupExampleInput" class="form-label">Partai</label>
                <input class="form-control" type="text" value="{{ $form_dprdkab->partai }}" aria-label="Disabled input example" disabled readonly>
              </div>
              <div class="mb-3">
                <label for="formGroupExampleInput" class="form-label">Kota</label>
                <input class="form-control" type="text" value="{{ $form_dprdkab->kota }}" aria-label="Disabled input example" disabled readonly>
              </div>
              <div class="mb-3">
                <label for="formGroupExampleInput" class="form-label">Provinsi</label>
                <input class="form-control" type="text" value="{{ $form_dprdkab->provinsi }}" aria-label="Disabled input example" disabled readonly>
              </div>
              <div class="mb-3">
                <label for="formGroupExampleInput" class="form-label">Kode Pos</label>
                <input class="form-control" type="text" value="{{ $form_dprdkab->kode_pos }}" aria-label="Disabled input example" disabled readonly>
              </div>
            </div>
            <div class="col-md-6" style="margin-top: 50px">
              <p  style="margin-top: 30px"><strong>Visi & Misi:</strong></p>
              <p>{{ $form_dprdkab->visi_misi }}</p>
              <p  style="margin-top: 50px"><strong>Pendidikan:</strong></p>
              <p>{{ $form_dprdkab->pendidikan }}</p>
              <p  style="margin-top: 50px"><strong>Pengalaman Kerja:</strong></p>
              <p>{{ $form_dprdkab->pengalaman_kerja }}</p>
              <p  style="margin-top: 50px"><strong>Organisasi:</strong></p>
              <p>{{ $form_dprdkab->organisasi }}</p>
              <p  style="margin-top: 50px"><strong>Prestasi:</strong></p>
              <p>{{ $form_dprdkab->prestasi }}</p>
          </div>
        </div>
        <div class="row">
          <div class="col-md-12" style="margin-top: 20px;">
              <!-- Tambahkan tombol Edit -->
              <a href="{{ route('portofolio/dprdkab/edit', $form_dprdkab->id) }}" class="btn btn-primary">Edit</a> 
              <!-- Tambahkan tombol Delete -->
              <form action="{{ route('destroy', $form_dprdkab->id) }}" method="POST" style="display: inline-block;">
                @csrf
                @method('DELETE')
